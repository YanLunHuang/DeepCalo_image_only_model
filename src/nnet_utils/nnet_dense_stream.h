...
void dense_ss_test(
      hls::stream<data_T> data[1],
      hls::stream<res_T>  res[1],
      typename CONFIG_T::weight_t weights_1[2][CONFIG_T::n_in*CONFIG_T::n_out/2],
      typename CONFIG_T::bias_t   biases[CONFIG_T::n_out]) {
      ...
      const int block_factor = DIV_ROUNDUP(CONFIG_T::n_out,reuse);
      #pragma HLS ARRAY_RESHAPE variable=weights_1 block factor=block_factor dim=2
      #pragma HLS ARRAY_PARTITION variable=biases complete
     ...
     for(int i_in = 0; i_in < CONFIG_T::n_in; i_in++) {
        for (int iacc = 0; iacc < reuse; iacc++) {
          ...
          for (int iacc2 = 0; iacc2 < block_factor; iacc2++) {
            #pragma HLS UNROLL
            unsigned x_index  =  i_in + (CONFIG_T::n_in*iacc2); 
			typename CONFIG_T::weight_t ww;
			if(iacc == 0)ww = weights_1[0][x_index];
			else ww = weights_1[1][x_index];
            acc[iacc2][iacc] += product_dense<data_T, typename CONFIG_T::weight_t, typename CONFIG_T::accum_t>(tmpdata[i_in], ww);
          }
      }
     }
     ResWrite:for (int iacc = 0; iacc < reuse; iacc++) {
